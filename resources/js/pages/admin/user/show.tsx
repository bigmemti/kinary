        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <DashboardContainer>
                ...
                <DataContainer>
                    <UserMeta user={user} />
                    {!!user.wallet && (
                        <WalletInfo user={user} wallet={user.wallet} />
                    )}
                    {!!user.student && (
                        <StudentInfo user={user} student={user.student} />
                    )}
                    {!!user.teacher && (
                        <TeacherInfo user={user} teacher={user.teacher} />
                    )}
                </DataContainer>
            </DashboardContainer>
        </AppLayout>
...
function TeacherActions({ user }: { user: User }) {
    return (
        <ActionButtonContainer>
            <ButtonLink href={teacher(user).url}>Teacher</ButtonLink>
...
function StudentMeta({ student }: { student: Student }) {
    return (
        <>
            ...
            <InfoBlock
                label="Student Enrollments Count"
                value={student.enrollments_count}
            />
        </>
    );
}
...
function StudentActions({ user }: { user: User }) {
    return (
        <ActionButtonContainer>
            <ButtonLink href={student(user).url}>Student</ButtonLink>
...
function WalletActions({ user }: { user: User }) {
    return (
        <ActionButtonContainer>
            <ButtonLink href={wallet(user).url}>Wallet</ButtonLink>
            <ButtonLink href={orders(user.wallet ?? 0).url}>Orders</ButtonLink>
        </ActionButtonContainer>
    );
}
    ...
    return (
        <>
            ...
            <InfoBlock
                ...
                operator="?"
                value={<CheckX condition={!!user.email_verified_at} />}
            />
            <InfoBlock
                label="Tow Factor"
                operator="?"
                value={<CheckX condition={!!user.two_factor_confirmed_at} />}
            />
...
function UserActions({ user }: { user: User }) {
    return (
        <ActionButtonContainer>
            {!(user.teacher || user.student || user.wallet) && (
                <FormButton
                    className="inline"
