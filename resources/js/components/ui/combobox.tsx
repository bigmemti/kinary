...
function ComboboxInput({
  ...
  return (
    <InputGroup className={cn("cn-combobox-input w-auto cursor-pointer", className)}>
    ...
    <ComboboxPrimitive.Portal>
      ...
      >
        <ComboboxPrimitive.Popup
          ...
          data-chips={!!anchor}
          className={cn(
            "cn-combobox-content cn-combobox-content-logical cn-menu-target cn-menu-translucent bg-black group/combobox-content relative max-h-(--available-height) w-(--anchor-width) max-w-(--available-width) min-w-[calc(var(--anchor-width)+--spacing(7))] origin-(--transform-origin) data-[chips=true]:min-w-(--anchor-width)",
            className
          )}
          {...props}
        />
...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        "cn-combobox-list overflow-y-auto overscroll-contain bg-black",
        className
      )}
      {...props}
    />
...
function ComboboxItem({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "cn-combobox-item relative flex w-full items-center outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 cursor-pointer",
        className
      )}
      {...props}
    >
